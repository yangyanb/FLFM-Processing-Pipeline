function  imshow3D( Img, disprange, currentColormap, labels )
...
nDims = ndims(Img);
if nDims==3
    currSlice = squeeze(Img(:,:,S,:));
    axes('position',[0,0.2,1,0.8]); imagesc(currSlice);%imshow(squeeze(Img(:,:,S,:)), [Rmin Rmax])
    pbaspect([1,1,1]);
    if(currentColormap)
...
% -=< Slice slider callback function >=-
    function SliceSlider (hObj,event, Img)
        ...
        if nDims==3
            currSlice = squeeze(Img(:,:,S,:));
            set(get(gca,'children'),'cdata',currSlice);
            caxis([min(currSlice(:)) max(currSlice(:))]);
            if(currentColormap)
                colormap(currentColormap);
            end
        else
