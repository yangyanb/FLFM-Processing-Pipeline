function y = conv_fft3(x, m, shape)
%   CONV_FFTn N dimensional convolution via the FFT.
%   Y = CONV_FFT3(X, M) performs the 3-D convolution of matrices X and M.
...
%
%   Y = CONV_FFT3(X, M, SHAPE) where SHAPE is a string returns a
...
narginchk(2,3);
if nargin < 3
    shape = 'full';    % shape default as for CONV3
end
...
y = ifftn(fftn(x) .* fftn(m));   % central operation, basic form

% trim to correct output size
if ~isequal(fsize, size(y))
    y = y(1:fsize(1), 1:fsize(2), 1:fsize(3));
end
...
switch shape
     ...
     case 'full'
        ...
        x = exindex3(x, 1:fsize(1), 1:fsize(2), {0});
        m = exindex3(m, 1:fsize(1), 1:fsize(2), {0});
    
    case 'valid'
        fsize = xsize - msize + 1;
        m = exindex3(m, 1:xsize(1), 1:xsize(2),1:xsize(3), {0}); % pad m with zeros 
        % shift m so that y(1,1) corresponds to mask just inside x
        me = msize + xsize - 1;
        m = exindex3(m, msize(1):me(1), msize(2):me(2), msize(3):me(3),'circular');
    case 'same'
        fsize = xsize;
        mmid = floor(msize/2);
        xsize = xsize + mmid;   % border to avoid edge effects
        x = exindex3(x, 1:xsize(1), 1:xsize(2), 1:xsize(3),{0});
        m = exindex3(m, 1:xsize(1), 1:xsize(2), 1:xsize(3),{0});
        % recentre m so that y(1,1) corresponds to mask centred on x(1,1)
        mc = 1 + mmid;
        me = mc + xsize - 1;
        m = exindex3(m, mc(1):me(1), mc(2):me(2),mc(3):me(3), 'circular');
    ...
    case 'wrap'
        ...
        if any(msize < xsize)  % test, as user may have optimised already
            m = exindex33(m, 1:xsize(1), 1:xsize(2), {0});
        end
        % recentre m so that y(1,1) corresponds to mask centred on x(1,1)
        mc = 1 + floor(msize/2);
        me = mc + xsize - 1;
        m = exindex3(m, mc(1):me(1), mc(2):me(2), 'circular');
    ...
    case 'reflect'
        ...
        xe = xc + xsize - 1;
        x = exindex3(x, xc(1):xe(1), xc(2):xe(2), 'symmetric');
        m = exindex3(m, 1:xsize(1), 1:xsize(2), {0});
        % recentre m so that y(1,1) corresponds to mask centred on x(1,1)
        me = msize + xsize - 1;
        m = exindex3(m, msize(1):me(1), msize(2):me(2), 'circular');
